...
const ERROR_WALLET_EXEC = 'Failed to start Petcoin-service. Set the path to Petcoin-service properly in the settings tab.';
...
WalletShellManager.prototype.startService = function(walletFile, password, onError, onSuccess, onDelay){
    ...
    childProcess.execFile(this.serviceBin, serviceArgs, (error, stdout, stderr) => {
            ...
            if(error){
                ...
                log.debug(stdout);
                if(stdout && stdout.length && stdout.indexOf('XPET') !== -1){
                    let trimmed = stdout.trim();
                    let walletAddress = trimmed.substring(trimmed.indexOf('XPET'), trimmed.length);
...
WalletShellManager.prototype._spawnService = function(walletFile, password, onError, onSuccess, onDelay){
    ...
    try{
        ...
        log.error('Petcoin-service is not running');
        return false;
    }
    ...
    function testConnection(retry){
        wsm.serviceApi.getAddress().then((address) => {
            ...
            if(retry === 10 && onDelay) onDelay(`Still no respond from Petcoin-service, please wait a few more seconds...`);
...
WalletShellManager.prototype.networkStateUpdate = function(state){
    ...
    if(state === 0){
        ...
        setImmediate(() => {
            ...
            setTimeout(()=>{
                log.debug('respawning Petcoin-service');
