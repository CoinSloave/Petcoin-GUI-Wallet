...
function showIntegratedAddressForm(){
    ...
    <textarea id="genInputAddress" class="default-textarea" placeholder="Required, put any valid XPET address..">${ownAddress}</textarea>
    </div>
    <div class="input-wrap">
...
function insertSampleAddresses(){
    ...
    const sampleData = [
        { name: 'javalopes',
          address: 'XPETapJnyjUSnA2QRfFJPs7V92hLAFKCRCAQzHqoU35HBMz6cZrJM3ESsm8f24gLJyfvpBc5MbYFF3CWs9DWm9ri9D1egFKtTA',
          paymentId: '', 
        },
        { name: '9CR9',
          address: 'XPETaKjSii5ioYkhHPcQ9idKNnYJE3MWSeF678nYR8sb1p6CAzvgvJRWD6nUBM7yuV65gVuWXo1QaQEH4PiMuPwpAM92dDqRii',
          paymentId: '', 
        },
        { name: 'sacananervos0',
          address: 'XPETSG5bsr63pVzWfKCFTcfiECajyhJdeRFkNCHqu48AZm44Lzyw6KCj4Aitji7Ad9jAzuwZnp7nYUA88To4MWHA4n9JSDxdKC',
          paymentId: '', 
        }
    ];
...
function handleSettings(){
    settingsButtonSave.addEventListener('click', function(){
        ...
        if(!wsutil.isRegularFileAndWritable(serviceBinValue)){
            formMessageSet('settings','error',`Unable to find Petcoin-service, please enter the correct path`);
            return false;
        }
...
function handleAddressBook(){
    ...
    addressBookButtonSave.addEventListener('click', () => {
        ...
        if(!wsutil.validateTRTLAddress(walletValue)){
            formMessageSet('addressbook','error',"Invalid Petcoin address");
            return;
        }
...
function handleSendTransfer(){
    ...
    sendButtonSend.addEventListener('click', () => {
        ...
        if(!recAddress.length || !wsutil.validateTRTLAddress(recAddress)){
            formMessageSet('send','error','Sorry, invalid XPET address');
            return;
        }
            ...
            <div class="div-transaction-panel">
                ...
                <div class="transferDetail">
                    ...
                    <dl>
                        ...
                        <dd class="dd-ib">${rAmount} XPET</dd>
                        <dt class="dt-ib">Transaction Fee:</dt>
                        <dd class="dd-ib">${rFee} XPET</dd>
                        <dt class="dt-ib">Node Fee:</dt>
                        <dd class="dd-ib">${(nodeFee > 0 ? nodeFee : '0.00')} XPET</dd>
                        <dt class="dt-ib">Total:</dt>
                        <dd class="dd-ib">${tobeSent} XPET</dd>
                    </dl>
                </div>
            </div>
            <div class="div-panel-buttons">
...
function initHandlers(){
    ...
    wsutil.liveEvent('#doGenIntegratedAddr', 'click', () => {
        ...
        if(!wsutil.validateTRTLAddress(addr)){
            formMessageSet('gia','error', 'Invalid XPET address');
            return;
        }
        // only allow standard address
        if(addr.length > 99){
            formMessageSet('gia','error', 'Only standard XPET address are supported');
            return;
        }
