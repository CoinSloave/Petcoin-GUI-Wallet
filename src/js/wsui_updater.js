...
function updateSyncProgres(data){
    ...
    if(knownBlockCount === SYNC_STATUS_NET_CONNECTED){
        ...
        if(connNodeFee > 0 ){
            connStatusText += ` | Node fee: <strong>${connNodeFee.toFixed(2)} XPET</strong>`;
        }
...
function updateBalance(data){
    ...
    let wintitle = `(${walletFile}) - ${bUnlocked} XPET`;
...
function showFeeWarning(fee){
    ...
    let htmlStr = `
        ...
        <p>The fee for sending transactions is: <strong>${fee.toFixed(2)} XPET </strong>.<br>
